...
namespace System.Buffers
{
    ...
    internal sealed class SlabMemoryPool : MemoryPool<byte>
    {
        ...
        /// </summary>
        private readonly ConcurrentQueue<SlabMemoryPoolBlock> _blocks = new ConcurrentQueue<SlabMemoryPoolBlock>();
        ...
        /// </summary>
        private SlabMemoryPoolBlock AllocateSlab()
        {
            ...
            SlabMemoryPoolBlock block = null;

            for (int i = 0; i < blockCount; i++) {
                block = new SlabMemoryPoolBlock(this, slab, offset, _blockSize);
        ...
        internal void Return(SlabMemoryPoolBlock block)
        {
#if BLOCK_LEASE_TRACKING
        ...
        internal void RefreshBlock(MemoryPoolSlab slab, int offset, int length)
        {
            lock (_disposeSync) {
                if (!_isDisposed && slab != null && slab.IsActive) {
                    ...
                    Return(new SlabMemoryPoolBlock(this, slab, offset, length));
                }
            }
        }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            lock (_disposeSync) {
                ...
                while (_blocks.TryDequeue(out SlabMemoryPoolBlock block)) {
                    GC.SuppressFinalize(block);
                }
            }
        }
